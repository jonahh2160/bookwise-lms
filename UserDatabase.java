...
public class UserDatabase extends Table<User> {
    ...
    // Edit an entry in the user array
    public void setEntry(User user, String fullName, String username, String password, String emailAddress, double accountBalance,
    boolean accountActive, int perms, String primaryKey) {
        ...
        if (accountBalance != -1) {user.setAccountBalance(accountBalance);}
        user.setActive(accountActive);
        if (perms != -1) {user.setPerms(perms);}
        if (primaryKey != null) {user.setPrimaryKey(primaryKey);}
    }
