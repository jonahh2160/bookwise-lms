...
public class Searcher {
    ...
    public ArrayList<Transaction> searchTransaction(int category, String term) {
        ...
        // Set everything to lowercase (for comparison reasons)
        String strSearch = term.toLowerCase();
        // Loop through transactions and if any of them match the search term, add it to
        // the list and return the list of searched transactions
    ...
    private String getTransactionSearchTerm(int category, int entry, String term) {
        ...
        if (category == 1) {
            strCompare = transactionDatabase.getEntry(entry).getBook().getTitle().toLowerCase();
        }
        if (category == 2) {
            strCompare = transactionDatabase.getEntry(entry).getDateDue().toLowerCase();
        }
        if (category == 3) {
            strCompare = transactionDatabase.getEntry(entry).getPrimaryKey().toLowerCase();
        }
